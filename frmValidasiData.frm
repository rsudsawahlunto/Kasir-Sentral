...
Private Sub subLoadData(Optional s_Kriteria As String)
    ...
    For i = 1 To rs.RecordCount
        With fgData
            ...
            sqlQuery = "SELECT dbo.FB_TakeStatusDataValid('" & .TextMatrix(i, 0) & "','" & .TextMatrix(i, 11) & "','" & .TextMatrix(i, 10) & "', '" & Format(.TextMatrix(i, 3), "yyyy/mm/dd HH:mm:ss ") & "', " & msubKonversiKomaTitik(CCur(.TextMatrix(i, 5))) & ", " & msubKonversiKomaTitik(CCur(.TextMatrix(i, 6))) & " , " & msubKonversiKomaTitik(CCur(.TextMatrix(i, 7))) & ", " & msubKonversiKomaTitik(CCur(.TextMatrix(i, 8))) & ", " & msubKonversiKomaTitik(CCur(.TextMatrix(i, 9))) & " )  as StatusData"
...
Private Sub cmdPerbaiki_Click()
    ...
    With fgData
        Set rs = Nothing
        strSQL = "select dbo.FB_TakeRuanganAsal('" & mstrNoPen & "','" & .TextMatrix(.Row, 11) & "', null,'" & Format(.TextMatrix(.Row, 3), "yyyy/mm/dd HH:mm:ss") & "','TM') "
        Call msubRecFO(rs, strSQL)
        ...
        If .TextMatrix(.Row, 12) = "09" Or .TextMatrix(.Row, 12) = "10" Or .TextMatrix(.Row, 12) = "16" Then
            ...
            If Add_TempHargaKomponenForPenunjang(.TextMatrix(.Row, 11), .TextMatrix(.Row, 3), .TextMatrix(.Row, 10), .TextMatrix(.Row, 13), .TextMatrix(.Row, 14), .TextMatrix(.Row, 6), .TextMatrix(.Row, 15), .TextMatrix(.Row, 16), "", sKdRuanganAsal) = False Then Exit Sub
        ...
        ElseIf .TextMatrix(.Row, 11) = "401" Or .TextMatrix(.Row, 11) = "402" Or .TextMatrix(.Row, 11) = "403" Then
            ...
            If rs.EOF = False Then
                sqlQuery = "Delete TempHargaKomponen where NoPendaftaran ='" & mstrNoPen & "' AND KdRuangan ='" & .TextMatrix(.Row, 11) & "'  and TglPelayanan ='" & Format(.TextMatrix(.Row, 3), "yyyy/MM/dd HH:mm:ss") & "' and KdPelayananRS = '" & .TextMatrix(.Row, 10) & "' " ' "
                Call msubRecFO(rsQuery, sqlQuery)
                If Add_TempHargaKomponenForIBS_DBNew(.TextMatrix(.Row, 11), .TextMatrix(.Row, 3), .TextMatrix(.Row, 10), .TextMatrix(.Row, 13), .TextMatrix(.Row, 14), .TextMatrix(.Row, 15), sKdRuanganAsal) = False Then Exit Sub
            Else
                sqlQuery = "Delete TempHargaKomponen where NoPendaftaran ='" & mstrNoPen & "' AND KdRuangan ='" & .TextMatrix(.Row, 11) & "'  and TglPelayanan ='" & Format(.TextMatrix(.Row, 3), "yyyy/MM/dd HH:mm:ss") & "' and KdPelayananRS = '" & .TextMatrix(.Row, 10) & "' " ' "
                Call msubRecFO(rsQuery, sqlQuery)
                If Add_TempHargaKomponenForIBSNew(.TextMatrix(.Row, 11), .TextMatrix(.Row, 3), .TextMatrix(.Row, 10), .TextMatrix(.Row, 13), .TextMatrix(.Row, 14), .TextMatrix(.Row, 15), sKdRuanganAsal) = False Then Exit Sub
            End If

        Else
            sqlQuery = "Delete TempHargaKomponen where NoPendaftaran ='" & mstrNoPen & "' AND KdRuangan ='" & .TextMatrix(.Row, 11) & "'  and TglPelayanan ='" & Format(.TextMatrix(.Row, 3), "yyyy/MM/dd HH:mm:ss") & "' and KdPelayananRS = '" & .TextMatrix(.Row, 10) & "' " ' "
            Call msubRecFO(rsQuery, sqlQuery)
            If Add_TempHargaKomponen(.TextMatrix(.Row, 11), .TextMatrix(.Row, 3), .TextMatrix(.Row, 10), .TextMatrix(.Row, 13), .TextMatrix(.Row, 14), .TextMatrix(.Row, 6), .TextMatrix(.Row, 15), .TextMatrix(.Row, 16), .TextMatrix(.Row, 17), sKdRuanganAsal) = False Then Exit Sub
...
Private Sub cmdTutup_Click()
    Unload Me
    frmTagihanPasien.cmdBayar.Enabled = True
End Sub
...
Public Function Add_TempHargaKomponenForPenunjang(f_KdRuangan As String, f_tglPelayanan As Date, f_KdPelayananRS As String, f_KdKelas As String, f_KdJenisTarif As String, f_TarifCito As Integer, f_JmlPelayanan As Integer, f_StatusCito As String, f_Kdlaboratory As String, f_KdRuanganAsal As String) As Boolean
    On Error GoTo errLoad
...
Public Function Add_TempHargaKomponen(f_KdRuangan As String, f_tglPelayanan As Date, f_KdPelayananRS As String, f_KdKelas As String, f_KdJenisTarif As String, f_TarifCito As Currency, f_JmlPelayanan As Integer, f_StatusCito As String, f_kdDokter As String, f_KdRuanganAsal As String) As Boolean
    On Error GoTo errLoad
    ...
    With dbcmd
        ...
        .Parameters.Append .CreateParameter("KdRuangan", adChar, adParamInput, 3, f_KdRuangan)
        .Parameters.Append .CreateParameter("TglPelayanan", adDate, adParamInput, , Format(f_tglPelayanan, "yyyy/MM/dd HH:mm:ss"))
        .Parameters.Append .CreateParameter("KdPelayananRS", adChar, adParamInput, 6, Left(f_KdPelayananRS, 6))
        .Parameters.Append .CreateParameter("KdKelas", adChar, adParamInput, 2, f_KdKelas)
        .Parameters.Append .CreateParameter("KdJenisTarif", adChar, adParamInput, 2, f_KdJenisTarif)
        .Parameters.Append .CreateParameter("TarifCito", adInteger, adParamInput, , f_TarifCito)
        .Parameters.Append .CreateParameter("JmlPelayanan", adInteger, adParamInput, , f_JmlPelayanan)
        ...
        .ActiveConnection = dbConn
        .CommandText = "dbo.Add_TempHargaKomponen"
        .CommandType = adCmdStoredProc
        .Execute
...
Public Function Add_TempHargaKomponenOAResep(f_NoPendaftaran As String, f_KdRuangan As String, f_tglPelayanan As Date, f_KdBarang As String, f_KdAsal As String, f_SatuanJml As String, f_HargaSatuan As Currency, f_HargaBeli As Currency, f_JmlBarang As Double, f_KdJenisObat As String, f_JmlService As Integer, f_TarifService As Currency, f_NoResep As String, f_BiayaAdministrasi As Currency, f_KdRuanganAsal As String, f_StatusDijamin As String, f_NoTerima As String, f_ResepKe As Integer) As Boolean
    On Error GoTo errLoad

    Add_TempHargaKomponenOAResep = True
    Set dbcmd = New ADODB.Command
    With dbcmd
        .Parameters.Append .CreateParameter("return_value", adInteger, adParamReturnValue, , Null)
        ...
        .Parameters.Append .CreateParameter("JmlBarang", adDouble, adParamInput, , CDec(f_JmlBarang))
        ...
        .Parameters.Append .CreateParameter("NoResep", adVarChar, adParamInput, 15, IIf(f_NoResep = "", Null, f_NoResep))
        .Parameters.Append .CreateParameter("BiayaAdministrasi", adCurrency, adParamInput, , f_BiayaAdministrasi)
        ...
        .Parameters.Append .CreateParameter("ResepKe", adInteger, adParamInput, , f_ResepKe) 'add resep ke

        .ActiveConnection = dbConn
        .CommandText = "dbo.Add_TempHargaKomponenOAResepNew"
        .CommandType = adCmdStoredProc
        .Execute
        ...
        If .Parameters("return_value").Value <> 0 Then
            MsgBox "Ada kesalahan dalam penyimpanan data ", vbCritical, "Validasi"
            Add_TempHargaKomponenOAResep = False
        End If
    End With
    Set dbcmd = Nothing
    Call deleteADOCommandParameters(dbcmd)
...
Public Function Add_TempHargaKomponenForIBS_DBNew(f_KdRuangan As String, f_tglPelayanan As Date, f_KdPelayananRS As String, f_KdKelas As String, f_KdJenisTarif As String, f_JmlPelayanan As Integer, f_KdRuanganAsal As String) As Boolean
    On Error GoTo errLoad
    ...
    With dbcmd
        ...
        .ActiveConnection = dbConn
        .CommandText = "dbo.Add_TempHargaKomponenForIBS_DB"
        .CommandType = adCmdStoredProc
        .Execute
...
Public Function Add_TempHargaKomponenForIBSNew(f_KdRuangan As String, f_tglPelayanan As Date, f_KdPelayananRS As String, f_KdKelas As String, f_KdJenisTarif As String, f_JmlPelayanan As Integer, f_KdRuanganAsal As String) As Boolean
    On Error GoTo errLoad
    ...
    With dbcmd
        ...
        .ActiveConnection = dbConn
        .CommandText = "dbo.Add_TempHargaKomponenForIBS"
        .CommandType = adCmdStoredProc
        .Execute
