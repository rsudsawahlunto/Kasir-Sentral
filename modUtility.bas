...
Public Sub msubPesanCetak()
    On Error GoTo hell
    If MsgBox("Apakah anda ingin langsung mencetak laporan?" & vbNewLine _
        & "Pilih No jika ingin ditampikan terlebih dahulu", vbQuestion + vbYesNo, "Konfirmasi") = vbYes Then
        vCetakLaporan = "Print"
    Else
        vCetakLaporan = "Preview"
    End If
hell:
End Sub
    ...
    Select Case mstrChoose
        Case "TglLahir"
            ...
            If Format(mobjObject.Text, "yyyy/MM/dd") > Format(Now, "yyyy/MM/dd") Then
                ...
                Exit Function
            End If
            '        Call subYearOldCount(mobjObject.Text)
...
Public Function funcRoundDownSplakuk(strNumber As String) As Double
    If InStr(strNumber, ".") = 0 And InStr(strNumber, ",") = 0 Then
        funcRoundDownSplakuk = CDbl(strNumber)
        Exit Function
    End If
    If InStr(strNumber, ".") <> 0 Then
        funcRoundDownSplakuk = CDbl(Left(strNumber, InStr(strNumber, ".")))
        Exit Function
    End If
    If InStr(strNumber, ",") <> 0 Then
        funcRoundDownSplakuk = CDbl(Left(strNumber, InStr(strNumber, ",")))
...
'seting visible menu
Public Sub SetVisibleMenu(strKdAplikasi As String, strNamaForm As String, strNamaMenu As Object, strStatus As String, strKategoryUser As String)
    strSQL = "SELECT * " & _
    " FROM StatusObject " & _
    " WHERE KdAplikasi= '" & strKdAplikasi & "' AND NamaForm= '" & strNamaForm & "' AND NamaObject= '" & strNamaMenu.Name & "' AND StatusEnable= '" & strStatus & "'AND KdKategoryUser='" & strKategoryUser & "'"
    msubRecFO rs, strSQL
    If rs.RecordCount = 1 Then
        strNamaMenu.Enabled = True

    Else
        strNamaMenu.Enabled = False
    End If
End Sub
